...
func (s *ProjectionService) HandleSettlementCreated(
	...
	log.Printf("HandleSettlementCreated: tenant=%s event_id=%s trace_id=%s occurred_at=%s settlement_id=%s batch_id=%s corridor_id=%s source_type=%s source_strength=%s source_system=%s source_system_id=%s bank_id=%s parse_conf=%.2f amount=%s currency=%s date=%s utr=%s rrn=%s bank_ref=%s provider_ref=%s client_ref=%s richness=%.2f readiness=%.2f status=%s ingest_run_id=%s",
		e.TenantID, e.EventID, e.TraceID, e.OccurredAt,
		e.SettlementID, e.BatchID, e.CorridorID, e.SourceType, e.SourceStrength, e.SourceSystem, e.SourceSystemID, e.BankID, e.ParseConfidence,
