...
export function WhyPaymentsNeedReviewChart({ amb }: WhyPaymentsNeedReviewChartProps) {
  const data = useMemo(() => {
    ...
    return [
      { label: 'Review Rate', pct: amb ? amb.ambiguity_rate * 100 : 0, color: '#34d399', pending: false },
      { label: 'Low Confidence', pct: lowConf * 100, color: '#a7f3d0', pending: false },
      { label: 'Missing Refs', pct: amb ? amb.provider_ref_missing_rate * 100 : 0, color: '#059669', pending: false },
      {
        label: 'Multi-Match',
