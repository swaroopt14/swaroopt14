...
export function usePaymentOperationsView(batchId?: string): {
  ...
  const viewModel = useMemo((): PaymentOperationsViewModel => {
    ...
    let hero = WORKSPACE_HERO_COPY.empty
    ...
    if (leakageOk && leakage.value_date_mismatch_count != null && leakage.value_date_mismatch_count > 0) {
      reviewBreakdown.push({
        label: 'Value-date mismatches',
        value: formatCount(leakage.value_date_mismatch_count),
      })
    }
