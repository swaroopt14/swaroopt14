...
if [ "$RESP" = "200" ]; then log_pass "zord-edge health" "HTTP 200"
...
if [ "$RESP" = "200" ]; then log_pass "zord-intent-engine health" "HTTP 200"
...
if [ "$RESP" = "200" ]; then log_pass "zord-outcome-engine health" "HTTP 200"
...
if [ "$RESP" = "200" ]; then log_pass "zord-evidence health" "HTTP 200"
...
if [ "$RESP" = "200" ]; then log_pass "zord-intelligence health" "HTTP 200"
...
if [ "$RESP" = "200" ]; then log_pass "zord-prompt-layer health" "HTTP 200"
...
if [ "$RESP" = "200" ]; then log_pass "zord-relay health" "HTTP 200"
...
if [ -n "$TENANT_ID" ]; then
  ...
  if [ "$INTEL_HTTP" = "200" ]; then
    HAS_DATA=$(echo "${INTEL_BODY}" | jq -r '.projections // .items // empty | length')
    log_pass "Intelligence KPIs" "HTTP 200, projections: ${HAS_DATA:-0}"
  elif [ "$INTEL_HTTP" = "401" ]; then
    log_pass "Intelligence KPIs" "Auth required (service is alive)"
  else
    log_fail "Intelligence KPIs" "Expected 200/401, got ${INTEL_HTTP}"
  fi
else
