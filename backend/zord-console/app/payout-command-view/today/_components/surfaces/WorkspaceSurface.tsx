...
import type { PromptLayerVisualization } from '@/services/payout-command/types'
...
function PromptLayerVisualizationBlock({
  visualization,
}: {
  visualization: PromptLayerVisualization
}) {
  if (visualization.type === 'timeline') {
    const maxValue = Math.max(...visualization.series.map((point) => point.v), 1)
    return (
      <div className="mt-3 rounded-[12px] border border-[#dce5ef] bg-[#f5f9ff] p-3">
        <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#6f85a7]">Timeline</div>
        <div className="flex h-20 items-end gap-1.5">
          {visualization.series.map((point) => (
            <div key={`${point.t}-${point.v}`} className="flex flex-1 flex-col items-center gap-1">
              <div
                className="w-full rounded-[4px] bg-[#79a4e6]"
                style={{ height: `${Math.max(8, (point.v / maxValue) * 60)}px` }}
                title={`${point.t}: ${point.v}`}
              />
              <span className="text-[10px] text-[#6f85a7]">{point.v}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  if (visualization.type === 'bars' || visualization.type === 'bar') {
    const maxValue = Math.max(...visualization.items.map((item) => item.value), 1)
    return (
      <div className="mt-3 rounded-[12px] border border-[#dce5ef] bg-[#f5f9ff] p-3">
        <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#6f85a7]">Bars</div>
        <div className="space-y-2">
          {visualization.items.map((item) => (
            <div key={`${item.label}-${item.value}`}>
              <div className="mb-1 flex items-center justify-between text-[12px] text-[#5d6e87]">
                <span>{item.label}</span>
                <span>{item.value}</span>
              </div>
              <div className="h-2 rounded-full bg-white">
                <div
                  className="h-2 rounded-full bg-[#79a4e6]"
  ...
  if (visualization.type === 'cards') {
    return (
      <div className="mt-3 rounded-[12px] border border-[#dce5ef] bg-[#f5f9ff] p-3">
        <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#6f85a7]">Cards</div>
        <div className="grid gap-2 sm:grid-cols-2">
          {visualization.cards.map((card) => (
            <div key={`${card.title}-${card.value}`} className="rounded-[10px] border border-[#d4deec] bg-white p-2.5">
              <div className="text-[11px] uppercase tracking-[0.08em] text-[#7a8da9]">{card.title}</div>
              <div className="mt-1 text-[15px] font-semibold text-[#2f4462]">{card.value}</div>
            </div>
          ))}
        </div>
      </div>
...
export function WorkspaceSurface({
  ...
  latestAnswerVisualization: PromptLayerVisualization | null
        ...
        <div className="grid gap-4 xl:grid-cols-[0.98fr_0.84fr] xl:grid-rows-[1fr_auto]">
          <article className="flex min-h-[33.5rem] flex-col overflow-hidden rounded-[1.7rem] border border-[#cfdaea] bg-[#DDE8F8] shadow-[0_12px_28px_rgba(0,0,0,0.05)]">
            <div className="relative px-6 pt-6">
              <div className="max-w-[12rem] text-[14px] font-medium leading-8 tracking-[0.01em] text-[#5c7194]">
                {scenario.heroLabel}
              </div>
              <div className="mt-5 text-[4.68rem] font-light tracking-[-0.06em] text-[#111111]">{scenario.heroValue}</div>
          ...
          <div className="flex h-full flex-col gap-4 xl:row-span-2">
            <article className="rounded-[1.6rem] border border-[#E5E5E5] bg-white p-5 shadow-[0_8px_22px_rgba(0,0,0,0.05)]">
              <div className="text-[14px] font-medium tracking-[0.01em] text-[#6f716d]">{scenario.listTitle}</div>
              ...
              <div className="mt-8 flex items-center justify-between gap-4">
                <div className="text-[14px] text-[#7a7a76]">{scenario.listFooter}</div>
                <button type="button" className="rounded-[1rem] border border-black/15 bg-[#f7f7f4] px-4 py-2.5 text-[14px] text-[#111111]">
                  {scenario.listAction}
                </button>
              </div>
            </article>
            ...
            <article className="rounded-[1.6rem] border border-[#E5E5E5] bg-white p-5 shadow-[0_8px_22px_rgba(0,0,0,0.05)]">
              <div className="text-[14px] font-medium tracking-[0.01em] text-[#6f716d]">{scenario.statTitle}</div>
              <div className="mt-5 text-[3.87rem] font-light tracking-[-0.06em] text-[#111111]">{scenario.statValue}</div>
              <div className="mt-2 text-[14px] leading-6 text-[#7a7a76]">{scenario.statNote}</div>
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <div className="rounded-[0.95rem] border border-black/10 bg-[#f8f8f6] px-3 py-3">
                  ...
                  <div className="mt-2 text-center text-[14px] text-[#7a7a76]">{scenario.compareLabels[0]}</div>
                </div>
                <div className="rounded-[0.95rem] border border-[#cfdaea] bg-[#bdd0ea] px-3 py-3">
                  ...
                  <div className="mt-2 text-center text-[14px] text-[#446ea7]">{scenario.compareLabels[1]}</div>
                </div>
              </div>
            </article>
            ...
            <article className="flex flex-1 flex-col rounded-[1.6rem] border border-[#d9e3f1] bg-gradient-to-br from-white via-[#f8fbff] to-[#eef4fc] p-5 shadow-[0_10px_24px_rgba(0,0,0,0.05)]">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#61789b]">Recovery lane brief</div>
                  <p className="mt-3 text-[14px] leading-6 text-[#5f6f85]">
                    Overflow cleared through healthier partner lanes while maintaining callback trust and finance-proof continuity.
                  </p>
                </div>
                <span className="inline-flex items-center gap-1.5 rounded-full border border-[#4ADE80]/35 bg-[#4ADE80]/14 px-2.5 py-1 text-[12px] font-medium text-[#166534]">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2">
                <div className="rounded-[0.9rem] border border-[#d4dfef] bg-white px-3 py-2.5">
                  <div className="text-[12px] text-[#7f8da2]">Stable</div>
                  <div className="mt-1 text-[16px] font-semibold text-[#27456f]">3 PSPs</div>
                </div>
                <div className="rounded-[0.9rem] border border-[#d4dfef] bg-white px-3 py-2.5">
                  <div className="text-[12px] text-[#7f8da2]">Lag risk</div>
                  <div className="mt-1 text-[16px] font-semibold text-[#27456f]">Low</div>
                </div>
                <div className="rounded-[0.9rem] border border-[#d4dfef] bg-white px-3 py-2.5">
                  <div className="text-[12px] text-[#7f8da2]">Proof-ready</div>
                  <div className="mt-1 text-[16px] font-semibold text-[#27456f]">142</div>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-3">
                {[
                  ...
                  <div key={label}>
                    <div className="mb-1.5 flex items-center justify-between text-[12px] text-[#6f7f96]">
                      <span>{label}</span>
                      <span>{progress}%</span>
                    </div>
          ...
          <article className="rounded-[1.6rem] border border-[#E5E5E5] bg-white p-5 shadow-[0_8px_22px_rgba(0,0,0,0.05)]">
            <div className="text-[14px] font-medium tracking-[0.01em] text-[#6f716d]">{scenario.bottomTitle}</div>
            <div className="mt-4 flex items-end justify-between gap-4">
              <div className="text-[3.33rem] font-light tracking-[-0.05em] text-[#111111]">{scenario.bottomValue}</div>
            ...
            <div className="mt-4 max-w-[30rem] text-[14px] leading-7 text-[#6f716d]">
              {scenario.bottomMeta}
            </div>
          </article>
        </div>
          ...
          <div className="mt-5 flex flex-1 flex-col rounded-[1.5rem] border border-[#E5E5E5] bg-white px-4 py-5 sm:px-5">
            <div className="border-b border-[#E5E5E5] pb-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="max-w-[28rem]">
                  <div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#8a8a86]">AI Intelligence Layer</div>
                  <div className="mt-2 text-[1.18rem] font-medium tracking-[-0.03em] text-[#111111]">
                    Route posture, owner handoff, and proof readiness in one reasoning layer.
                  </div>
                </div>
                <div className="inline-flex items-center gap-2 rounded-full border border-[#4ADE80]/28 bg-[#4ADE80]/14 px-3 py-2 text-[13px] font-medium text-[#14532d]">
              ...
              <div className="mt-5 rounded-[1.5rem] border border-[#ebeae4] bg-[linear-gradient(180deg,#fcfcfa_0%,#f8f8f5_100%)] p-4 sm:p-5">
                <div className="flex flex-col gap-3 border-b border-[#ebeae4] pb-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#166534]">
                      <span className="h-2 w-2 rounded-full bg-[#4ADE80]" />
                  ...
                  <div className="flex flex-wrap gap-2 text-[12px] text-[#7d7e79]">
                ...
                >
                  {conversation.map((message, index) => (
                    <div key={message.id} className={`flex gap-3 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                      ...
                      <div className={`${message.role === 'user' ? 'max-w-[88%]' : 'max-w-[92%]'}`}>
                        <div className={`mb-2 flex items-center gap-2 text-[12px] ${message.role === 'user' ? 'justify-end text-[#8a8a86]' : 'text-[#8a8a86]'}`}>
                        ...
                        <div className={`rounded-[18px] px-4 py-3.5 text-[15px] leading-7 ${messageBubbleClass(message)}`}>
                          {message.body}
                        </div>

                        {message.role === 'assistant' && message.citationSnippet ? (
                          <div className="mt-2 rounded-[12px] border border-[#dce5ef] bg-[#f5f9ff] px-3 py-2.5 text-[13px] leading-5 text-[#5d6e87]">
                            <div className="mb-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#6f85a7]">Supporting context</div>
                        ...
                        {message.role === 'assistant' && (message.confidence || message.hasVisualization) ? (
                          <div className="mt-2 flex flex-wrap gap-2">
                            {message.confidence ? (
                              <div className="inline-flex rounded-full border border-[#4ADE80]/35 bg-[#4ADE80]/16 px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.08em] text-[#166534]">
                                {message.confidence}
                              </div>
                            ) : null}
                            {message.hasVisualization ? (
                              <div className="inline-flex rounded-full border border-[#d4deec] bg-white px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.08em] text-[#5d6e87]">
                                Chart payload ready
                              </div>
                            ) : null}
                          </div>
                        ...
                        {message.role === 'assistant' && message.visualization ? (
                          <PromptLayerVisualizationBlock visualization={message.visualization} />
                        ) : null}
                      </div>

                      {message.role === 'user' ? (
                        <div className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-[11px] bg-[#111111] text-[12px] font-semibold text-white shadow-[0_10px_18px_rgba(0,0,0,0.14)]">
                          OP
                        </div>
                      ) : null}
                    </div>
                  ...
                  {!hasUserConversation && latestMessage ? (
                    <div className="rounded-[16px] border border-dashed border-[#e6e4de] bg-white/70 px-4 py-3 text-[13px] leading-6 text-[#8a8a86]">
              ...
              {!hasUserConversation ? (
                <>
                  <div className="mt-5">
                    <div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#8a8a86]">Suggested Questions</div>
                  ...
                  <div className="mt-5 rounded-[1.2rem] border border-[#cfdaea] bg-[#eaf1fc] p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#5c7194]">{latestAnswerTitle}</div>
                      {latestAnswerConfidence ? (
                        <span className="rounded-full border border-[#4ADE80]/35 bg-[#4ADE80]/16 px-2 py-1 text-[11px] font-medium uppercase tracking-[0.08em] text-[#166534]">
                          {latestAnswerConfidence}
                        </span>
                      ) : null}
                    </div>
                    <div className="mt-3 text-[15px] leading-7 text-[#243550]">
                      {latestAnswerStatus === 'loading' ? 'Reading prompt-layer evidence…' : latestAnswerBody}
                    </div>
                    {latestAnswerCitationSnippet ? (
                      <div className="mt-3 rounded-[10px] border border-[#d4deec] bg-white/70 px-3 py-2 text-[13px] leading-5 text-[#5d6e87]">
                        {latestAnswerCitationSnippet}
                      </div>
                    ) : null}
                    {latestAnswerVisualization ? <PromptLayerVisualizationBlock visualization={latestAnswerVisualization} /> : null}
                  </div>
                </>
              ) : null}
            </div>
            ...
            {!hasUserConversation ? (
              <div className="mt-5 flex-1">
                <div className="mb-3 text-[11px] font-semibold uppercase tracking-[0.1em] text-[#8a8a86]">Operator Modules</div>
                    ...
                    <article key={tile.title} className="rounded-[1.25rem] border border-[#E5E5E5] bg-[#F7F7F4] px-5 py-5 shadow-[0_10px_24px_rgba(0,0,0,0.04)]">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="text-[1.13rem] font-medium tracking-[-0.03em] text-[#111111]">{tile.title}</div>
                          <p className="mt-3 text-[14px] leading-6 text-[#6f716d]">{scenario.moduleBodies[index] ?? tile.body}</p>
                        </div>
                      </div>
                    </article>
                  ))}
              ...
              <div className="min-w-0 flex-1">
                <div className="rounded-[1rem] border border-[#3b82f6]/80 bg-[#1d1d1d] px-4 py-4 shadow-[inset_0_0_0_1px_rgba(59,130,246,0.18)]">
                  <input
                    ...
                    className="w-full bg-transparent text-[19px] font-medium tracking-[-0.02em] text-white/92 outline-none placeholder:text-white/52 sm:text-[21px]"
                  />
                </div>
              </div>
